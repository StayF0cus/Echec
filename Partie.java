...
public class Partie {
  ...
  public void alternanceJoueur() {
    if(this.joueur == "B") this.joueur = "N";
    else this.joueur = "B";
   }
  ...
  public int[] choixCase(){
    ...
    while(saisieOk != true){ // saisie controler
        ...
        selection = sc.nextLine();
          ...
          char sauvegarder = selection.charAt(0); // met la premier lettre de ce qui y'a dans selection
        ...
        if(selection.length() == 2){
            ...
            colonneI = Character.getNumericValue(colonneC); // convertit
             if( (colonneI >= 1 && colonneI <= 8) && (ligneI >= 1 && ligneI <= 8) ){ //verifie les veleur donnee
                 ...
                 int[] cases = {ligneI-1,colonneI-1}; // pour remetre en valeur tableau
                 return cases;
              }
        }
    }
  ...
  public void jouer() {
    while(this.echiquier.testPat(joueur) != true && this.echiquier.testMat(joueur) != true){ //
      ...
      int[] tab = this.choixCase(); // on met le choix d'une case dans un tableau
      Piece p = this.echiquier.getCase(tab[0],tab[1]);
